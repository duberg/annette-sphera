package annette.imc.notification.client

import java.net.URLEncoder

import annette.imc.notification.SmsSettings
import annette.imc.notification.model.Notification
      ...
      Try(request).flatMap { x =>
        if (x.isError) {
          ...
          decode[ResponseJson](x.body)
            .fold(_ => Failure(SmsClient.JsonProtocolException(s"Invalid json response {{${x.body}}")), y => {
              y.result match {
