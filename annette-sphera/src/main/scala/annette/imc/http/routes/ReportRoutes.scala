      ...
      (path("get-info-all") & get & auth) { sessionData =>
        ...
        (path("generate" / JavaUUID / JavaUUID) & get & auth) { (apId, reportId, sessionData) =>
          parameter('format.?) { x =>
            ...
            val f = for {
              ...
              x <- reportService.generate(reportId, apId, userId, Map.empty, format, user.defaultLanguage)
            } yield x
            onComplete(f) {
        ...
        (path("generate" / JavaUUID / JavaUUID) & post & auth & entity(as[Map[String, String]])) { (apId, reportId, sessionData, parameters) =>
          parameter('format.?) { x =>
            ...
            val f = for {
              user <- coreModule.userDao.getById(userId).map(_.get)
              x <- reportService.generate(reportId, apId, userId, parameters, format, user.defaultLanguage)
            } yield x
            onComplete(f) {
              case Success(source) =>
                complete(HttpResponse(
        ...
        (path("bulletin-for-expert" / JavaUUID / JavaUUID / JavaUUID) & get & auth) { (reportId, apId, expertId, sessionData) =>
          ...
          val f = for {
            ...
            x <- reportService.generate(reportId, apId, expertId, Map.empty, format, user.defaultLanguage)
          } yield x
          onComplete(f) {
            case Success(source: Source[ByteString, Future[IOResult]]) =>
              complete(HttpResponse(
