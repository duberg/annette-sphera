    ...
    (id, sessionData, updateUser) =>
      ...
      val f = for {
        ...
        val user = UserUpdate(
          ...
          defaultLanguage = Some(updateUser.user.defaultLanguage))

        val imcUser = ImcUser(
          id = id,
          sphere = updateUser.sphere,
          ...
          userOpt.flatMap(_.email).foreach(email => {
            for {
              ...
              notificationService.addNotificationAsync(MailNotification.Password(
                ...
                email = email,
                password = password,
                language = userOpt.map(_.defaultLanguage).get,
                templateParameters = p))
            }
          })
          ...
          case Some(b) =>
            for {
              ...
              UpdatedUser(
                id,
                PreUser(
                  a.lastname, a.firstname, a.middlename, a.email, a.phone, a.defaultLanguage),
                None,
                b.sphere,
                b.company,
                b.position,
