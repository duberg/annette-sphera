...
class UserServiceSpec extends TestKit(ActorSystem("UserActorSpec"))
  ...
  "A UserDao" when {
    "createUser" must {
      ...
      //
      //          cc1 <- dao.createUser(c1)
      //          cc2 <- recoverToExceptionIf[UserAlreadyExists](dao.createUser(c2))
      //        } yield {
    ...
    //        val dao = newUserDao()
    //        for {
    //          cc1 <- dao.createUser(c1, "abc")
    //          cc2 <- dao.updateUser(u1)
    ...
    //        for {
    //          cc1 <- dao.createUser(c1, "")
    //          cc2 <- recoverToExceptionIf[LoginRequired](dao.updateUser(u1))
    //        } yield {
    //          cc1 shouldBe ()
    ...
    //        val dao = newUserDao()
    //        for {
    //          cc1 <- recoverToExceptionIf[UserNotFound](dao.updateUser(u1))
    //        } yield {
    ...
    //        val dao = newUserDao()
    //        for {
    //          cc1 <- dao.createUser(c1).mapTo[CreateUserSuccess].map(_.x)
    //          cc2 <- dao.createUser(c2)
