...
//      userExist <- userDao.getUserById(userId).map(_.nonEmpty)
//    } yield {
...
//        Future.sequence(applicationIds.map(a => applicationDao.getApplicationById(a))).map(_.flatten)
//    }
//    val languagesFuture = tenantsFuture.flatMap {
//      tenants =>
//        val languageIds = tenants.flatMap(_.languages)
//        Future.sequence(languageIds.map(a => languageDao.getLanguageById(a))).map(_.flatten)
//    }
//
//    for {
